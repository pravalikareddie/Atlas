...
export async function deletePlaceExperience(id: string): Promise<void> {
  const { error } = await supabase
    .from('place_experiences')
    .delete()
    .eq('id', id)
...
export async function updateActivity(
  ...
  const { error } = await supabase.from('activities').update(u).eq('id', id)
  if (error) throw error
}
...
export async function deleteLivingTodo(id: string): Promise<void> {
  const { error } = await supabase.from('living_todos').delete().eq('id', id)
  if (error) throw error
}
